import { MoodId, MOOD_IDS } from './simulationConfig';
...
export function generateMoodRibbon(
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    const breathPulse = Math.sin(breathProgress * Math.PI * 2 + t * Math.PI) * config.ribbonPulseAmount;
    ...
    const innerRadius = baseRadius * config.presenceRadius - halfWidth + breathPulse + wave;
    const outerRadius = baseRadius * config.presenceRadius + halfWidth + breathPulse + wave;
...
export function isInRibbonZone(
  ...
  const maxWidth = config.ribbonBaseWidth + Math.log(1001) * config.ribbonScaleFactor; // Max possible width
